...
A complete, standalone Node.js/TypeScript CLI tool for cleaning and managing WordPress installations. Designed as a modern replacement for the original Clean Sweep 2.0, this tool can be safely called from WordPress frontend via `exec()` / `shell_exec()` on restricted shared hosting.

## Key Features

- **100% Standalone** - No WP-CLI or PHP dependencies required
- **Safe by Default** - All commands default to `--dry-run` mode
- **JSON Output** - Always outputs clean JSON for easy PHP parsing
- **Malware Detection** - Signature-based detection with 100+ patterns
- **Core Repair** - Download and replace WordPress core files
- **Plugin Management** - Reinstall official WordPress.org plugins
- **Database Tools** - Scan and optimize WordPress databases
- **Security Auditing** - Permissions, config validation, hardening checks
- **Scheduling** - Automated periodic scans via cron
...
git clone https://github.com/your-username/clean-sweep-cli.git
cd clean-sweep-cli
...
export PATH="/path/to/clean-sweep-cli/bin:$PATH"
```
...
# Scan a WordPress installation (dry-run by default)
clean-sweep scan --path /var/www/html --json

# Check WordPress health status
clean-sweep status --path /var/www/html

# Full scan with all detection features
clean-sweep scan \
  --path /var/www/html \
...
# Enable a cron job
clean-sweep cron:manage enable job-id

# Disable a cron job
clean-sweep cron:manage disable job-id
```
...
Since this is a standalone CLI, you can call it from PHP:

```php
<?php
// From WordPress theme or plugin
$output = shell_exec('/path/to/clean-sweep-cli/bin/clean-sweep scan --path /var/www/html --json');
$result = json_decode($output, true);

if ($result['safe']) {
    echo "Site is clean!";
} else {
    echo "Threats found: " . count($result['threats']);
}
?>
```
...
npm test              # Run tests
npm run build:pkg     # Create standalone binaries (requires pkg)
```
...
clean-sweep-cli/
├── bin/                    # CLI entry point
│   └── clean-sweep         # Executable script
├── src/
│   ├── commands/           # All command implementations
│   ├── malware-scanner.ts  # Core malware detection
